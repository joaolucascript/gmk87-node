...
(function () {
  function buildBridge() {
    ...
    window.gmk87 = {
      ...
      windowGetBounds: async () => {
        try {
          const data = await invoke("window_get_bounds");
          return { success: true, data };
        } catch (error) {
          return { success: false, error: String(error) };
        }
      ...
      windowGetMonitorCenter: async () => {
        try {
          const data = await invoke("window_get_monitor_center");
          return { success: true, data };
        } catch (error) {
          return { success: false, error: String(error) };
        }
      ...
      windowSetCenterSize: async (opts) => {
        try {
          await invoke("window_set_center_size", {
            width: opts.width,
            height: opts.height,
            centerX: opts.centerX,
            centerY: opts.centerY,
            chromeW: opts.chromeW,
            chromeH: opts.chromeH,
          });
          return { success: true };
        } catch (error) {
          return { success: false, error: String(error) };
      ...
      windowSetInnerSize: async (width, height) => {
        try {
          await invoke("window_set_inner_size", { width, height });
          return { success: true };
        } catch (error) {
          return { success: false, error: String(error) };
        }
      ...
      windowSetMinInnerSize: async (width, height) => {
        try {
          await invoke("window_set_min_inner_size", { width, height });
          return { success: true };
        } catch (error) {
          return { success: false, error: String(error) };
        }
