...
const UploadView = {
  ...
  _setUploadProgress(percent, status) {
    ...
    textEl.textContent = `${percent}% — ${status}`;
    btn.innerHTML = `<span class="spinner"></span> ${percent}%`;
  },
  ...
  async _upload() {
    ...
    const removeProgressListener = window.gmk87.onUploadProgress(({ percent, status }) => {
